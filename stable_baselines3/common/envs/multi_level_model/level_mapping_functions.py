...
def get_accmap(fine_grid,coarse_grid):
    ...
    ind_x = np.sort(np.array(list(range(fine_grid.ny)))%coarse_grid.ny)
    ind_y = np.sort(np.array(list(range(fine_grid.nx)))%coarse_grid.nx)
    xv,yv = np.meshgrid(ind_x,ind_y)
    accmap = np.dstack((xv.reshape(fine_grid.nx,fine_grid.ny,1),yv.reshape(fine_grid.nx,fine_grid.ny,1))) 
